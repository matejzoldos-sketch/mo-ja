-- Predaj dashboard: tržby / AOV / LTV v net (bez DPH).

CREATE OR REPLACE FUNCTION public.shopify_gross_to_net_for_order(
  p_order_id bigint,
  p_gross numeric
)
RETURNS numeric
...
REVOKE ALL ON FUNCTION public.shopify_gross_to_net_for_order(bigint, numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.shopify_gross_to_net_for_order(bigint, numeric) TO service_role;
GRANT EXECUTE ON FUNCTION public.shopify_gross_to_net_for_order(bigint, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.shopify_gross_to_net_for_order(bigint, numeric) TO anon;
...
-- get_shopify_dashboard_summary: 4 net conversions
CREATE OR REPLACE FUNCTION public.get_shopify_dashboard_summary(p_range text DEFAULT 'ytd'::text, p_kpi_product text DEFAULT NULL::text, p_month text DEFAULT NULL::text, p_year text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- shopify_dashboard_kpis_for_window: 2 net conversions
CREATE OR REPLACE FUNCTION public.shopify_dashboard_kpis_for_window(p_ts_from timestamp with time zone, p_ts_to_excl timestamp with time zone, p_kpi_product text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
