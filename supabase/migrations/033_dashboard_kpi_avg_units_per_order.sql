-- KPI avg_units_per_order = SUM(line_item.quantity) / COUNT(paid-ish orders) in the same window.
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: KPIs incl. avg_units_per_order (line qty sum / paid-ish order count), returning_customers_pct 2+ in window; ytd|30d|90d|365d';
