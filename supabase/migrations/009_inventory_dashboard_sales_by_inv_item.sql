-- Match YTD sales to inventory rows by inventory_item_id first (line items often lack SKU
-- or use title; raw_json inventoryItemSku can differ). Fallback to SKU/title label.
...
sold_by_inv_item AS (
  ...
  WHERE li.inventory_item_id IS NOT NULL
    AND (o.created_at AT TIME ZONE 'Europe/Bratislava')::date BETWEEN b.d0 AND b.d1
...
joined AS (
  ...
  LEFT JOIN sold_by_sku sy ON sy.sku_label = inv.sku
)
SELECT COALESCE(
  json_agg(
    json_build_object(
...
COMMENT ON FUNCTION public.get_shopify_inventory_dashboard IS
  'Inventory × location + YTD avg daily units sold (paid-ish): match sales by inventory_item_id, else SKU/title label; estimated days = available / avg';
