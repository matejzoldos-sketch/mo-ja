-- Re-enable rolling 365-day window (label: last ~12 months on dashboard).
-- Same returning_customers_pct logic as 30d/90d (prior paid-ish before window).
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: sargable created_at range + returning_customers_pct (YTD repeat-in-window vs rolling prior-order); p_range ytd|30d|90d|365d (365d = today-364..today)';
