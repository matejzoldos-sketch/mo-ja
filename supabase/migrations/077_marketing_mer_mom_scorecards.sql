-- MER scorecards: predchádzajúce obdobie (MoM / YoY podľa filtra).
...
prev_bounds AS (
  SELECT
    CASE
      WHEN b.range_key = 'month' THEN
        (date_trunc('month', b.d_from::timestamp) - interval '1 month')::date
      WHEN b.range_key = 'year' THEN
        make_date(EXTRACT(YEAR FROM b.d_from)::int - 1, 1, 1)
      ELSE
        (b.d_from - ((b.d_to - b.d_from) + 1))::date
    END AS d_from,
    CASE
      WHEN b.range_key = 'month' THEN (b.d_from - 1)::date
      WHEN b.range_key = 'year' THEN
        make_date(EXTRACT(YEAR FROM b.d_from)::int - 1, 12, 31)
      ELSE (b.d_from - 1)::date
    END AS d_to,
    CASE
      WHEN b.range_key = 'month' THEN 'MoM'
      WHEN b.range_key = 'year' THEN 'YoY'
      ELSE 'Predch. obdobie'
    END AS compare_kind
  FROM bounds b
),
...
data_bounds AS (
  SELECT
    LEAST(make_date(2026, 1, 1), pb.d_from, b.d_from) AS d_from,
    GREATEST(b.d_to, pb.d_to) AS d_to
  FROM bounds b
...
COMMENT ON FUNCTION public.get_shopify_marketing_mer_dashboard(text, text, text) IS
  'MER dashboard: Shopify revenue + Meta CSV ads + journal fees; kpisPrevious for MoM/YoY scorecards.';
