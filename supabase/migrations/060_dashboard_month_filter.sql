...
BEGIN
  ...
  IF v_norm NOT IN ('ytd', '30d', '90d', '365d') THEN
    RAISE EXCEPTION 'invalid p_range: % (allowed: ytd, 30d, 90d, 365d, month)', p_range;
  END IF;
...
REVOKE ALL ON FUNCTION public.shopify_dashboard_date_bounds(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.shopify_dashboard_date_bounds(text, text) TO service_role;

COMMENT ON FUNCTION public.shopify_dashboard_date_bounds(text, text) IS
  'Bratislava date window for MOJA dashboards: rolling ranges or calendar month (p_month YYYY-MM).';

DROP FUNCTION IF EXISTS public.get_shopify_dashboard_mvp(text, text);

CREATE OR REPLACE FUNCTION public.get_shopify_dashboard_mvp(p_range text DEFAULT 'ytd', p_kpi_product text DEFAULT NULL, p_month text DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
BEGIN
  ...
  FROM public.shopify_dashboard_date_bounds(p_range, p_month) b;
...
REVOKE ALL ON FUNCTION public.get_shopify_dashboard_mvp(text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_shopify_dashboard_mvp(text, text, text) TO service_role;
...
REVOKE ALL ON FUNCTION public.get_shopify_sku_units_daily_ytd(text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_shopify_sku_units_daily_ytd(text, text, text) TO service_role;

COMMENT ON FUNCTION public.get_shopify_sku_units_daily_ytd(text, text, text) IS 'Daily units per top-10 canonical labels (shopify_product_display_label); paid_orders: has_product_line or KPI-matching line; p_range ytd|30d|90d|365d|month + optional p_month YYYY-MM';

ALTER FUNCTION public.get_shopify_sku_units_daily_ytd(text, text, text) VOLATILE;


CREATE OR REPLACE FUNCTION public.get_shopify_marketing_dashboard(p_range text DEFAULT '90d', p_month text DEFAULT NULL)
RETURNS json
LANGUAGE sql
STABLE
SECURITY DEFINER
...
range_ok AS (
  SELECT *
  FROM bounds b
  WHERE b.range_key IN ('30d', '90d', '365d', 'month')
...
REVOKE ALL ON FUNCTION public.get_shopify_marketing_dashboard(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_shopify_marketing_dashboard(text, text) TO service_role;

COMMENT ON FUNCTION public.get_shopify_marketing_dashboard(text, text) IS
  'Marketing UTM dashboard: KPIs, breakdown by source/medium/campaign, top orders; paid product orders; 30d|90d|365d|month + p_month YYYY-MM (Bratislava).';
