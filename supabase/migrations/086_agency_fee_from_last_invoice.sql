-- Agency fee for Spend dashboard: use last journal invoice as fixed monthly fee.
-- 14d window = monthly_fee × 14 / days_in_current_month.
-- YTD months: actual invoice if present, else forward-fill from last invoice month.
...
BEGIN
  ...
  -- Fixed monthly agency fee from the latest journal invoice.
  last_agency AS (
    SELECT
  ...
  -- Actual invoice if present; else forward-fill fixed fee from last invoice month onward.
  monthly_agency AS (
    SELECT
      m.month_key,
      CASE
  ...
  SELECT json_build_object(
    'meta', json_build_object(
      ...
      'notes', json_build_array(
        ...
        'Agency fee = fix z poslednej faktúry v denníku; 14d = mesiac × 14 / dni v mesiaci; YTD mesiace bez faktúry sa dopĺňajú dopredu.'
      )
    ),
    'decision', (
      SELECT json_build_object(
...
COMMENT ON FUNCTION public.get_executive_scaling_dashboard() IS
  'Executive spend decision: 14d traffic-light matrix + YTD monthly trends. Agency = last invoice fixed fee.';
