-- Marketing dashboard: férové porovnanie agentúr (celé aktívne obdobie + prvých N dní).
...
COMMENT ON FUNCTION public.get_shopify_marketing_dashboard(text, text, text) IS
  'Marketing UTM dashboard: KPIs, breakdowns, byAgency (filter window), agencyBenchmark (lifetime + first 30/35/60d).';
