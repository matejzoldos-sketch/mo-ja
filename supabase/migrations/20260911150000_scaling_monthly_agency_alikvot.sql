-- Scaling dashboard: prorate agency fee for current calendar month
-- (Europe/Bratislava) in monthly series used by "Reálna ziskovosť" chart.
-- Matches MTD scorecard alikvot. Completed months keep full monthly_fee.

CREATE OR REPLACE FUNCTION public.get_executive_scaling_dashboard(p_window text DEFAULT 'mtd'::text)
 RETURNS json
 LANGUAGE plpgsql
...
DECLARE
  ...
  -- Agency fee: journal > bank > temporary fix 1014.71 €/mes from 2026-09-01 (11. 8. bank amount) until invoice.
...
BEGIN
  ...
  -- Journal > bank > temporary fix 1014.71 € (11. 8.) until invoice. No forward-fill.
  agency_months AS (
  ...
  monthly_agency AS (
    SELECT
      m.month_key,
      CASE
        WHEN m.month_start = date_trunc('month', v_today::timestamp)::date
          THEN ROUND(
            COALESCE(af.monthly_fee, 0)
            * (
              (LEAST(m.month_end, v_today) - m.month_start + 1)::numeric
              / NULLIF((m.month_end - m.month_start + 1)::numeric, 0)
            ),
            2
          )
        ELSE COALESCE(af.monthly_fee, 0)
      END::numeric AS agency_fee
    FROM months m
  ...
  SELECT json_build_object(
    'meta', json_build_object(
      ...
      'notes', json_build_array(
        ...
        'Agency fee = denník > Tatra honzabartos > dočasný fix 1014.71 €/mes od 1. 9. 2026 (suma z 11. 8.), kým nepríde faktúra. FP 3260023 (1. 4.) = marec. Alikvot = fee mesiaca / dni × dni v okne; aktuálny mesiac aj v monthly sérii (alikvot MTD).'
      )
    ),
    'decision', (
      SELECT json_build_object(
