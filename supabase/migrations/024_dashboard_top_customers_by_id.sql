-- Top zákazníci podľa Shopify customer_id (bez mena/emailu v JSON); len paid-ish objednávky v rovnakom okne ako dashboard.
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: KPIs, daily revenue, top products, topCustomers (Shopify customer_id + orders + revenue, no PII), recent orders; returning_customers_pct via shopify_order_returning_group_key';
