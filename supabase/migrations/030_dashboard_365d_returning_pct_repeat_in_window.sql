-- 365d returning_customers_pct: % of distinct customers (group key) in window with
-- 2+ paid-ish orders in the same window — same counting branch as calendar YTD (rw_ytd).
  ...
  CROSS JOIN LATERAL (
    SELECT
      CASE
        ...
        WHEN v_norm IN ('ytd', '365d') THEN ROUND(100.0 * rw_ytd.cnt / cw.cnt, 1)
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: returning_customers_pct = ytd|365d: % with 2+ paid-ish orders in window; 30d|90d: % with prior paid-ish before window; p_range ytd|30d|90d|365d';
