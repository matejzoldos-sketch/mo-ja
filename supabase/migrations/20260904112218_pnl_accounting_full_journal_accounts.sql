-- Accounting P&L: expose every journal 5xx bucket (incl. 551 odpisy + residual).
-- COGS stays booked 504 only. Hybrid unchanged (XLS + 42%).
...
expense_monthly AS (
  SELECT
    ...
    ROUND(SUM(l.amount_eur) FILTER (WHERE l.debit_account ~ '^551'), 2) AS depreciation,
    ...
    ROUND(SUM(l.amount_eur) FILTER (
      WHERE l.debit_account ~ '^5'
        AND l.debit_account !~ '^(504|501|513|518|538|548|551|56)'
    ), 2) AS residual_opex,
...
monthly AS (
  SELECT
    ...
    COALESCE(e.depreciation, 0) AS depreciation,
    COALESCE(e.financial, 0) AS financial,
    COALESCE(e.residual_opex, 0) AS residual_opex,
...
totals AS (
  SELECT
    ...
    ROUND(SUM(residual_opex), 2) AS residual_opex,
...
SELECT json_build_object(
  'meta', json_build_object(
    ...
    'note', 'P&L čisto z denníka: všetky výnosy 6xx a náklady 5xx. COGS = 504; OPEX = ostatné 5xx vrátane odpisov 551. Staff/marketing = klasifikácia v rámci 518 (nie navyše). Účty mimo importu denníka (ak chýbajú v CSV) sa neobjavia.'
  ),
...
COMMENT ON FUNCTION public.get_pnl_dashboard(text) IS
  'P&L z denníka: všetky 6xx výnosy a 5xx náklady. COGS=504; odpisy=551; residual_opex = ostatné 5xx.';
