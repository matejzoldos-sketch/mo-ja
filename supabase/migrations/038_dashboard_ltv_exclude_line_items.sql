-- avg_customer_ltv: lifetime sum z položiek (qty * unit_price) bez vylúčených riadkov (lístky, MOJA fáza bez chaosu), nie z o.total_price.
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: KPIs incl. avg_customer_ltv from non-excluded line revenue (qty*unit_price), avg_units_per_order, pct_orders_multi_sku, returning_customers_pct; recentOrders 30d=newest, 90d/365d=top by total_price; ytd|30d|90d|365d';
