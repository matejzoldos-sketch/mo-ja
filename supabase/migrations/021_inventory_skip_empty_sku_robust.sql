-- Tabuľka + graf skladu: riadky bez SKU podľa raw_json.inventoryItemSku (nie porovnanie reťazca „—“).
-- Snapshoty: vyhodiť chr(8212)/chr(8211) a jednoznakové „-“.
...
COMMENT ON FUNCTION public.get_shopify_inventory_dashboard IS
  'Inventory × location + YTD avg daily units; only rows with non-empty raw_json.inventoryItemSku; paid-ish match; needs read_products + read_all_orders for full YTD';
...
COMMENT ON FUNCTION public.get_shopify_inventory_stock_chart_ytd IS
  'Daily last snapshot per SKU (top 10 by latest stock), 7 Apr–today Bratislava; placeholder-only sku_label omitted; forward-filled in UI';
