-- Sklad stock chart: x-axis range from 1 April (same calendar year as YTD) — Jan–Mar had no snapshots.
...
COMMENT ON FUNCTION public.get_shopify_inventory_stock_chart_ytd IS
  'Daily last snapshot per SKU (top 10 by latest stock), 1 Apr–today Bratislava; forward-filled in UI';
