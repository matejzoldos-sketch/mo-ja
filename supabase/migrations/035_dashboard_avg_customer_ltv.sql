-- KPI avg_customer_ltv = AVG(all-time paid-ish revenue per customer) among customers with ≥1 paid-ish order in the dashboard window (same identity as returning_customers_pct).
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: KPIs incl. avg_units_per_order, pct_orders_multi_sku, avg_customer_ltv (cohort lifetime avg via returning_group_key), returning_customers_pct 2+ in window; ytd|30d|90d|365d';
