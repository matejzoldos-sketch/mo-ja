-- Sklad: kanonické názvy produktov (shopify_product_display_label) — zladené s predaj dashboardom.
...
COMMENT ON FUNCTION public.get_shopify_inventory_stock_chart_ytd IS
  'YTD stock chart: points.product_title = shopify_product_display_label (aligned with predaj); points.sku stays snapshot key';
...
COMMENT ON FUNCTION public.get_shopify_inventory_dashboard IS
  'Inventory × location + YTD; product_title = shopify_product_display_label (aligned with predaj); estimated_stockout_date YYYY-MM-DD text';
