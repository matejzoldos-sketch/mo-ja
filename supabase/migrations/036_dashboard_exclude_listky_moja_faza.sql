-- Vylúčenie lístkov (názov/SKU) a produktu „MOJA fáza bez chaosu“ z položkových agregátov Predaj dashboardu a SKU grafu.
...
COMMENT ON FUNCTION public.shopify_line_item_excluded_from_predaj_dashboard(text, text) IS 'Predaj dashboard: vylúčenie lístkov a MOJA fáza bez chaosu (SKU alebo názov položky).';
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: KPIs incl. avg_units_per_order, pct_orders_multi_sku, avg_customer_ltv, returning_customers_pct 2+ in window; line aggregates exclude lístky and MOJA fáza bez chaosu (shopify_line_item_excluded_from_predaj_dashboard); ytd|30d|90d|365d';
...
COMMENT ON FUNCTION public.get_shopify_sku_units_daily_ytd(text) IS 'Daily line-item units per top-10 SKU labels; excludes lístky and MOJA fáza bez chaosu; p_range ytd|30d|90d|365d (Bratislava tz, sargable created_at)';
