-- Extend monthlyNewVsReturning with customer counts.

-- monthlyNewVsReturning: revenue + customer counts (lifetime first order).
    ...
    (
      ...
      win_ord AS (
        SELECT
          ...
          public.shopify_order_returning_group_key(o.raw_json, o.customer_id, o.customer_email) AS gk,
          (
            SELECT COALESCE(SUM(public.shopify_gross_to_net_for_order(li.order_id, li.quantity * COALESCE(li.unit_price, 0))), 0)::numeric
            FROM shopify_order_line_items li
            WHERE li.order_id = o.id
              AND public.shopify_line_item_included_for_dashboard_filter(li.sku, li.title, p_kpi_prod)
          ) AS rev
        FROM shopify_orders o
...
BEGIN
  ...
  CROSS JOIN LATERAL (
    SELECT (
      ...
      ltv_by_gk AS (
        SELECT public.shopify_order_returning_group_key(o.raw_json, o.customer_id, o.customer_email) AS gk,
               SUM(public.shopify_gross_to_net_for_order(li.order_id, li.quantity * COALESCE(li.unit_price, 0)))::numeric AS ltv_sum
        FROM shopify_orders o
  ...
  agg AS (
    SELECT (o.created_at AT TIME ZONE v_tz)::date AS day,
           COALESCE(SUM(public.shopify_gross_to_net_for_order(li.order_id, li.quantity * COALESCE(li.unit_price, 0))), 0)::numeric AS revenue
    FROM shopify_order_line_items li
    INNER JOIN shopify_orders o ON o.id = li.order_id
    WHERE o.created_at >= v_ts_from
      AND o.created_at < v_ts_to_excl
    ...
    (
      ...
      FROM (
        SELECT public.shopify_product_display_label(li.sku, li.title) AS label,
               SUM(public.shopify_gross_to_net_for_order(li.order_id, li.quantity * COALESCE(li.unit_price, 0)))::numeric AS revenue,
