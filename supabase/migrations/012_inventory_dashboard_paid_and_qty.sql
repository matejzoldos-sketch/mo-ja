-- Broader "paid-ish" order match (column or raw_json displayFinancialStatus, AUTHORIZED, safe %PAID%).
-- Line qty: prefer column; else currentQuantity / quantity from raw_json (Shopify snapshots).
-- eff inventory item: GID id or inventoryItem.legacyResourceId in raw_json.
-- SKU key: line sku, variant.sku, variantTitle, title, name (helps when variant is null but line has data).
...
COMMENT ON FUNCTION public.get_shopify_inventory_dashboard IS
  'Inventory × location + YTD avg daily units: paid-ish (incl. AUTHORIZED); qty from column or raw_json; inv item id from GID or legacyResourceId; SKU key from line/variant/title/name; needs read_products for variant on line items + read_all_orders for full YTD';
