-- MER: mROAS = Revenue / (Ads + agency management fees).
-- Agency fees = Správa PPC / Filip (fee_category=agency); not tools/creative (Canva, Mailer, BCreativum).

INSERT INTO public.marketing_expense_map (priority, match_supplier, match_text, bucket, fee_category, notes)
SELECT 10, 'žitňansk', NULL, 'fees', 'agency', 'PPC agentúra — Bc. Filip Žitňanský'
WHERE NOT EXISTS (
  SELECT 1 FROM public.marketing_expense_map m
  WHERE m.fee_category = 'agency' AND m.match_supplier = 'žitňansk'
);

INSERT INTO public.marketing_expense_map (priority, match_text, bucket, fee_category, notes)
SELECT 11, 'správa ppc', 'fees', 'agency', 'PPC správa kampaní (line text)'
WHERE NOT EXISTS (
  SELECT 1 FROM public.marketing_expense_map m
  WHERE m.fee_category = 'agency' AND m.match_text = 'správa ppc'
);
...
BEGIN
  ...
  -- Fallback: PPC management invoices (avoid bare "filip" — false positives).
  IF v_hay ~ 'správa\s*ppc|sprava\s*ppc|žitňansk|zitnansk' THEN
    RETURN true;
  END IF;
...
SELECT json_build_object(
  'meta', (
    SELECT json_build_object(
      ...
      'journal_note', 'Fees z denníka; Ads z Meta CSV. mROAS = Revenue / (Ads + Fees agentúry: Správa PPC).',
...
COMMENT ON FUNCTION public.get_shopify_marketing_mer_dashboard(text, text, text) IS
  'MER dashboard: MoM scorecards + mROAS = Revenue / (Ads + agency fees).';
