-- KPI pct_orders_multi_sku = % paid-ish orders with COUNT(DISTINCT sku/title label) > 1 in the same window.
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON: KPIs incl. avg_units_per_order, pct_orders_multi_sku (distinct SKU/title labels > 1), returning_customers_pct 2+ in window; ytd|30d|90d|365d';
