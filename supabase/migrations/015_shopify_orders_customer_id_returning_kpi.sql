-- Customer id on orders (sync_shopify GraphQL customer { id }) + KPI returning_customers_pct on dashboard RPC.

ALTER TABLE shopify_orders
  ADD COLUMN IF NOT EXISTS customer_id BIGINT;

COMMENT ON COLUMN shopify_orders.customer_id IS 'Shopify Customer legacy id from Order.customer.id (GID tail); NULL for guest';
...
BEGIN
  ...
  CROSS JOIN LATERAL (
    ...
    FROM (
      SELECT COUNT(DISTINCT o.customer_id)::numeric AS cnt
...
COMMENT ON FUNCTION public.get_shopify_dashboard_mvp(text) IS 'MVP JSON for MOJA dashboard: KPIs (+ returning_customers_pct), daily revenue, top 5 products, 10 recent orders; p_range ytd|30d|90d (Bratislava dates)';
