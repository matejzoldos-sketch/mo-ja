-- Lightweight summary RPC for faster dashboard first paint.

CREATE OR REPLACE FUNCTION public.get_shopify_dashboard_summary(p_range text DEFAULT 'ytd', p_kpi_product text DEFAULT NULL, p_month text DEFAULT NULL, p_year text DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
COMMENT ON FUNCTION public.get_shopify_dashboard_summary(text, text, text, text) IS
  'Lighter sales dashboard RPC: meta, KPI cards, daily revenue, top products and recent orders.';
