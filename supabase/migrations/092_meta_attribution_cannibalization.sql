...
DECLARE
  ...
  v_agency_fee_override_from CONSTANT date := DATE '2026-08-01';
  v_result json;
BEGIN
  WITH
  paid_base AS (
  ...
  -- YTD months: override from Aug 2026 (prorate incomplete current month);
  ...
  attribution_ytd AS (
    SELECT
      ...
      CASE
        WHEN (COALESCE(SUM(m.meta_click_sales), 0) + COALESCE(SUM(m.meta_view_sales), 0)) > 0
          THEN ROUND(
            100.0 * COALESCE(SUM(m.meta_view_sales), 0)
              / (COALESCE(SUM(m.meta_click_sales), 0) + COALESCE(SUM(m.meta_view_sales), 0)),
            1
          )
        ELSE NULL
    ...
    'meta', json_build_object(
      ...
      'attribution', (
        SELECT json_build_object(
          ...
          'view_through_warn', COALESCE(a.view_through_ratio_pct, 0) > 40,
...
COMMENT ON FUNCTION public.get_executive_scaling_dashboard() IS
  'Executive spend: 14d matrix + YTD. Attribution cannibalization (click vs view vs UTM).';
