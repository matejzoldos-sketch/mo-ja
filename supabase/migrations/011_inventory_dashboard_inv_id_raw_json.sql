-- Line items: inventory_item_id sometimes NULL in column but present in raw_json (variant.inventoryItem.id).
-- paid_order: also match displayFinancialStatus on stored order raw_json (enum string).
-- SKU keys: map fullwidth / small plus (U+FF0B, U+FE62) to ASCII + for PH+-B1-C style labels.
...
paid_order AS (
  ...
  WHERE (o.created_at AT TIME ZONE 'Europe/Bratislava')::date BETWEEN b.d0 AND b.d1
    AND (
      ...
      OR (o.raw_json->>'displayFinancialStatus') IN (
        'PAID',
        'PARTIALLY_PAID',
        'PARTIALLY_REFUNDED'
      )
  ...
  SELECT
    ...
    COALESCE(
      ...
      CASE
        ...
        THEN (
          SUBSTRING(li.raw_json #>> '{variant,inventoryItem,id}' FROM '([0-9]+)$')
        )::bigint
        ELSE NULL
      END
...
sold_by_inv_item AS (
  SELECT
    ...
    SUM(le.quantity)::numeric AS units_sold
  FROM line_eff le
...
COMMENT ON FUNCTION public.get_shopify_inventory_dashboard IS
  'Inventory × location + YTD avg daily units: paid-ish orders; sales by eff. inventory_item_id (column or raw_json), else normalized SKU; read_all_orders needed for >~60d order history in Shopify API';
