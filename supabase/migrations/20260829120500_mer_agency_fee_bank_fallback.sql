-- MER: when journal has no agency fee for a month, use Tatra honzabartos.cz debit as estimate.
...
SELECT json_build_object(
  'meta', (
    SELECT json_build_object(
      ...
      'journal_note', 'Fees z denníka (YTD Marketing & Promo); ak chýba agency fee, odhad z Tatra honzabartos.cz. Ads z Meta CSV. mROAS = Revenue / (Ads + Fees agentúry). VK Google je vo Fees, nie v mROAS.',
...
COMMENT ON FUNCTION public.get_shopify_marketing_mer_dashboard(text, text, text) IS
  'MER dashboard: journal fees + honzabartos bank fallback for missing agency fee months.';
