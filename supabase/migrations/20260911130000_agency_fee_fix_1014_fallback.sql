-- Temporary Honza Bartoš agency fee: journal > bank > 1014.71 €/mes from 2026-09-01
-- (amount from 2026-08-11 Tatra debit) until invoice lands.
-- Applies to executive scaling + MER dashboards.
...
DECLARE
  ...
  -- Agency fee: journal > bank (honzabartos) > temporary fix 1014.71 €/mes from 2026-09-01 (Aug 11 bank amount) until invoice.
  v_agency_fee_fix CONSTANT numeric := 1014.71;
  v_agency_fee_fix_from CONSTANT date := DATE '2026-09-01';
  v_result json;
BEGIN
  ...
  -- Journal > bank > temporary fix (Aug 11 honzabartos amount) until invoice. No forward-fill.
  agency_months AS (
  ...
  agency_fee_for_month AS (
    SELECT
      ...
      CASE
        ...
        WHEN COALESCE(b.monthly_fee, 0) > 0 THEN b.monthly_fee
        WHEN m.month_start >= v_agency_fee_fix_from THEN v_agency_fee_fix
        ELSE 0::numeric
      END AS monthly_fee,
      CASE
        WHEN COALESCE(j.monthly_fee, 0) > 0 THEN j.fee_as_of
        WHEN COALESCE(b.monthly_fee, 0) > 0 THEN b.fee_as_of
        WHEN m.month_start >= v_agency_fee_fix_from THEN m.month_start
        ELSE NULL
      END AS fee_as_of,
      CASE
      ...
      CASE
        WHEN COALESCE(j.monthly_fee, 0) > 0 THEN false
        WHEN COALESCE(b.monthly_fee, 0) > 0 THEN false
        WHEN m.month_start >= v_agency_fee_fix_from THEN true
        ELSE false
      END AS is_override
    FROM agency_months m
    LEFT JOIN agency_journal_by_month j ON j.month_start = m.month_start
    LEFT JOIN agency_bank_by_month b ON b.month_start = m.month_start
  ),
  last_agency AS (
    SELECT
      fee_as_of,
  ...
  SELECT json_build_object(
    'meta', json_build_object(
      ...
      'notes', json_build_array(
        ...
        'Agency fee = denník > Tatra honzabartos > dočasný fix 1014.71 €/mes od 1. 9. 2026 (suma z 11. 8.), kým nepríde faktúra. FP 3260023 (1. 4.) = marec. Alikvot = fee mesiaca / dni × dni v okne.'
      )
    ),
    'decision', (
      SELECT json_build_object(
...
COMMENT ON FUNCTION public.get_executive_scaling_dashboard(text) IS
  'Executive spend: window mtd|14d|30d + YTD. Agency = journal > bank > 1014.71€ fix from 2026-09-01. Meta CPA vs contrib. margin.';
...
SELECT json_build_object(
  'meta', (
    SELECT json_build_object(
      ...
      'journal_note', 'Fees z denníka (YTD Marketing & Promo); FP 3260023 (1. 4.) = marec. Ak chýba agency fee: Tatra honzabartos, inak od 1. 9. 2026 dočasný fix 1014.71 € (11. 8.). Ads z Meta CSV. mROAS = Revenue / (Ads + Fees agentúry). VK Google je vo Fees, nie v mROAS.',
